...
export function Logo({ size = 'md', href = '/', className = '' }: LogoProps) {
  ...
  const inner = (
    <span
      className={`inline-flex items-center ${s.gap} font-bold tracking-tight ${className}`}
      style={{ color: 'var(--text-primary)' }}
    >
      {/* Bolt mark — surface bg, primary bolt */}
      <span
        className={`relative inline-flex ${s.mark} items-center justify-center rounded-md`}
