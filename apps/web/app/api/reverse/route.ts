...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Initialize Convex client only when needed
    ...
    const result = await convex.action(api.promptforge.reverse, {
      prompt: body.prompt,
    });
    logger.info("reverse.api", { userId });
    return NextResponse.json(result, { headers: cors });
  } catch (e) {
    const msg = e instanceof Error ? e.message : "Reverse failed";
    logger.error("reverse.api.error", { err: msg });
