...
export default function LandingPage() {
  ...
  return (
    <main className="min-h-[100dvh] bg-[#fafafa] text-[#16161a]">
      ...
      <div className="border-b border-black/8 bg-[#fafafa] overflow-hidden py-2.5">
        <div
        ...
        >
          {[TICKER_ITEMS, TICKER_ITEMS].map((t, idx) => (
            <span
              key={idx}
              aria-hidden={idx > 0}
              className="text-[11px] font-semibold uppercase tracking-widest text-black/50 px-8"
            >
              {t}
            </span>
          ))}
      ...
      {/* ── B. Sticky nav ────────────────────────────────────────────── */}
      <nav className="sticky top-0 z-40 border-b border-black/8 bg-[#fafafa]/90 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
          <Logo size="md" />
          <div className="hidden md:flex items-center gap-8 text-sm font-medium text-black/40">
            <Link href="/showcase" className="hover:text-black transition-colors duration-150">Showcase</Link>
            <Link href="/benchmark" className="hover:text-black transition-colors duration-150">Benchmark</Link>
            <Link href="/install" className="hover:text-black transition-colors duration-150">Install</Link>
            <Link href="/pricing" className="hover:text-black transition-colors duration-150">Pricing</Link>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-6 pt-24 pb-20 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        >
          {/* Eyebrow */}
          <motion.div variants={stagger.item} className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-black/40 mb-8">
          ...
          {/* Subhead */}
          <motion.p variants={stagger.item} className="text-lg text-black/60 leading-relaxed max-w-[55ch] mb-10">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-black/8">
            {[
              ...
              >
                <p className="text-3xl md:text-4xl font-bold tracking-tighter text-[#16161a] mb-1">
                  {stat.value}
                </p>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        ...
        <div className="divide-y divide-black/8 border-b border-black/8">
          {FEATURES.map((f, i) => (
            <motion.div
              key={f.num}
              initial={{ opacity: 0, y: 12 }}
              ...
              className={`grid grid-cols-12 gap-4 items-center py-6 px-4 -mx-4 rounded-lg transition-colors duration-150 ${
                hoveredFeature === i ? 'bg-black/5' : ''
              }`}
            >
              <span className="col-span-1 text-xs font-semibold uppercase tracking-widest text-black/30">
                {f.num}
              </span>
              <span className="col-span-3 md:col-span-2 text-base font-semibold text-[#16161a]">
                {f.name}
              </span>
              <span className="col-span-8 md:col-span-7 text-sm text-black/60 leading-relaxed">
                {f.desc}
              </span>
              <span className="hidden md:block col-span-2 text-right">
                <span className="text-[10px] font-semibold uppercase tracking-widest text-black/20 border border-black/10 rounded-full px-2.5 py-1">
                  {f.tag}
                </span>
              </span>
            </motion.div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 divide-y md:divide-y-0 md:divide-x divide-black/8">
          {STEPS.map((step, i) => (
            ...
            >
              <p className="text-6xl font-bold tracking-tighter text-black/10 mb-6 leading-none">
                {step.num}
              </p>
              <h3 className="text-xl font-bold tracking-tight text-[#16161a] mb-3">
                {step.title}
              </h3>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        ...
        <div className="flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-black/8 border-y border-black/8">
          {SURFACES.map((s, i) => (
            <motion.div
              key={s.name}
              initial={{ opacity: 0, y: 12 }}
              ...
              viewport={{ once: true, margin: '-60px' }}
              transition={{ duration: 0.5, delay: i * 0.06, ease: EASE }}
              className="flex-1 px-6 py-8 group hover:bg-black/3 transition-colors duration-150"
            >
              <p className="text-base font-semibold text-[#16161a] mb-1 group-hover:text-black transition-colors">
                {s.name}
              </p>
      ...
      <section className="border-t border-black/8">
        <div className="max-w-7xl mx-auto px-6 py-32 text-center">
          ...
          >
            <p className="text-xs font-semibold uppercase tracking-widest text-black/40 mb-8">
              Get started
            </p>
            <h2 className="text-5xl md:text-7xl font-bold tracking-tighter leading-none text-[#16161a] mb-8">
              Stop fighting
              <br />
              with prompts.
            </h2>
            <p className="text-lg text-black/60 leading-relaxed max-w-[45ch] mx-auto mb-12">
